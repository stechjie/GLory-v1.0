...
const PresentationSlice := preload("res://effects/runtime/presentation/BattlePresentationSlice.gd")
...
func _collect_attack_events(current: Dictionary, ranged: bool) -> Array[Dictionary]:
	...
	for id: String in current.keys():
		...
		if int(now.get("attack_count", 0)) <= int(prev.get("attack_count", 0)):
			continue
		# D4 atomic migration: a unit in the vertical slice is drawn from Director
		# cues instead. Skipping it here is what stops the same swing playing twice.
		if PresentationSlice.is_migrated(str(now.get("unit_id", ""))):
			continue
...
func _play_visual_events(state_snapshot: Dictionary,current:Dictionary) -> void:
	...
	while _vfx_visual_event_index < events.size():
		...
		if str(event.get("type", "")) == "skill_shake":
			...
			# D4 atomic migration: numbers whose attacker is in the slice are drawn
			# by the adapter on the Director timing instead.
			if PresentationSlice.is_migrated(_slice_unit_id_of(current, str(event.get("source_uid", "")))):
				continue
...
# --- D4 vertical slice entry points -------------------------------------------
# Single-cue equivalents of the batched diff helpers above. The batch path stays
# untouched for every unit outside BattlePresentationSlice, so the two routes
# never overlap. _vfx_prev_units is reused deliberately: it already holds the
# most recent snapshot after each refresh, and the ping-pong buffer comment above
# forbids a second cross-frame holder of the same dictionaries.

func slice_unit_id_for(sim_uid: String) -> String:
	return _slice_unit_id_of(_vfx_prev_units, sim_uid)


func slice_play_basic_attack(source_uid: String, target_uid: String, ranged: bool) -> bool:
	var source: Dictionary = _slice_unit_snapshot(source_uid)
	if source.is_empty():
		return false
	var attack := {
		"id": source_uid,
	...
	var target: Dictionary = _slice_unit_snapshot(target_uid)
	if target.is_empty():
		target = _floor_target_for(attack)
	var race := _visual_race_from_unit_id(str(attack.get("unit_id", "")))
...
func slice_spawn_hit_number(target_uid: String, amount: int, kind: String, crit: bool, is_skill: bool, race: String) -> bool:
	var target: Dictionary = _slice_unit_snapshot(target_uid)
	if target.is_empty():
		return false
...
func slice_claim_corpses(events: Array) -> void:
	for event_value in events:
		...
		if uid.is_empty() or _slice_corpses.has(uid):
			continue
		if not PresentationSlice.is_migrated(_slice_unit_id_of(_vfx_prev_units, uid)):
			continue
		var claimed: Node3D = detach_actor_for_death(uid)
		if claimed != null:
			_slice_corpses[uid] = claimed


func slice_release_corpses() -> void:
	for uid in _slice_corpses.keys():
		var actor = _slice_corpses[uid]
		if actor != null and is_instance_valid(actor):
			release_death_actor(str(uid), actor as Node3D)
	_slice_corpses.clear()


func slice_play_death(victim_uid: String) -> bool:
	var actor: Node3D = _slice_corpses.get(victim_uid)
	_slice_corpses.erase(victim_uid)
	...
	tween.tween_property(actor, "position", sink, 0.32).set_ease(Tween.EASE_IN)
	tween.tween_property(actor, "scale", actor.scale * 0.72, 0.32).set_ease(Tween.EASE_IN)
	for child in actor.get_children():
		if child is GeometryInstance3D:
			tween.tween_property(child, "transparency", 1.0, 0.30)
	tween.chain().tween_callback(func() -> void: release_death_actor(victim_uid, actor))
	return true


func _slice_unit_snapshot(sim_uid: String) -> Dictionary:
	if sim_uid.is_empty():
		return {}
...
func _slice_unit_id_of(units: Dictionary, sim_uid: String) -> String:
	if sim_uid.is_empty():
		return ""
	var snapshot: Dictionary = units.get(sim_uid, {})
	if snapshot.is_empty():
		snapshot = _vfx_prev_units.get(sim_uid, {})
	return str(snapshot.get("unit_id", ""))
