...
> 状态（2026-08-19）：D0-D4 Windows/桌面闭环已完成，Android 部分按用户要求暂停；D5-D6 尚未完成。下方未勾选项仍是正式验收门槛，不能因已有基础设施而视为通过。
...
| D6 — 清理旧路由 | ⬜ 未开始 | **要清理的是 `BattleVfx` 的快照 diff 分支，不是事件分支**：普攻起手、受击、死亡本来就由 `_refresh_battle_vfx` 的 diff 驱动，只有 4 个类型走 `_play_visual_events`。因此 D6 是「删掉已迁移视觉的 diff 残留 + 删掉 `BattlePresentationSlice.gd`」，而不是先加事件、再统一删分支——中间态会每个效果播两遍。Legacy adapter 保留到覆盖完成并验证无重复播放、跨局残留或持续增长。 |

### 下一执行门

1. 进入 D5 手感与运行时预算：五个 `.tres` profile、`VfxProfileResolver`、扩展现有 `effects/vfx3d/core/VFXQualityBudget.gd`、`BattleVfxReview.tscn`，并把切片白名单从 3 个单位逐步放开到全部。D5 还必须补 §8 里「D5-D6 真实效果 seek/skip 测试」——D2 那批是空 adapter 跑的，接了真 Tween 和计时器之后必须重跑。
2. 事件哈希已由 D4 按流程重新冻结（新增 `attack_start`/`projectile_spawn`/`impact`/`death` 必然改变）：Round 1 `310426f76d73a80fb1d691bad711b61f1415fe4f2e7844776138f4beffacef1f`；Round 2 `7d2e60fcd4523fca8785d10cefb0e780a5b2b7a1217b241a11eedbdc9ea0f365`。D1 的旧值 `44e47d20…` / `4f134f2b…` 作为历史保留，D2、D3 在其上验证过不变。D5-D6 不得无记录地改变新值。
3. final-state 哈希继续作为严格不回归门禁，当前值：Round 1 `186f158728e2fdac189d2aac832e1da760af9644635e997b3bb3bb74a1cef505`；Round 2 `443d17206fd1edfe3e36e7ddfbd9c02a909c0d0b99feafe3b3ee0e584b7ea026`。**这两个值不是被 D4 改掉的**：本会话进行中的上游合并 `e50022b` 改了 `data/pve/pve_monsters.json` 的 `name_en`（如 `Ancient Tree` → `Elder Treant`），战斗帧逐行零差异。隔离实验（同一提交、撤回 D4）复现了同样的新值。D0 旧值 `6ff92454…` / `8ce66a51…` 作为历史保留。
...
| D6：清理旧路由 | 删除已迁移视觉的**快照 diff 分支**（不是事件分支）与 `BattlePresentationSlice.gd`，保留 Legacy adapter 直到覆盖完成。 | 无重复播放、无跨局残留、无节点/内存持续增长。 |
