...
#### 候选形状：座位票（⚠️ 未拍板，2026-09-09 记录）

> 这一段是讨论中推出来的**候选方案，不是已决议**。写进来是因为它给上面第一个问题
> 提供了一个具体形状，值得在拍板时摆上桌。采不采用等 `C14` 的方案定了再说。

第一个问题 —— 「JWT 怎么在 ENet 握手时交给 ③」—— 可能问错了。答案也许是**不交**：

```
客户端 ──JWT / HTTPS──→ ②     ② 签发一张座位票（短时效，绑 player_id + room + seat）
客户端 ──座位票 / ENet──→ ③     ③ 只验票，永远见不到 JWT
```

收益有三条：

- **ENet 上永远不出现账号凭证。** 🔴 第 1 条（JWT 不上 ENet）从结构上成立，不靠自觉。
- **③ 的验证面收窄。** 从「验一家 Auth 的 JWT」变成「验 ② 签的票」，`9.1` 在这个
  形状下好答得多。
- **爆炸半径可控。** 偷到票 = 抢一个座位、且只在 TTL 内有效；偷到 JWT = 丢号。

**但它不消掉 `C14`，也不消掉这条信任链：**

1. 票仍然在明文 ENet 上跑。短 TTL + 一次性 + 绑连接只能压小窗口，压不没。
2. ② 今天根本不知道房间存在 —— 房间全在 ③ 的内存里。要签票就得先有 ②↔③ 通道，
   于是绕回上面第三个问题（③ → ② 用什么服务端凭证）。

所以它是这三个问题的一个**候选答案，不是完成态**。上面那句「在 `C14` 落地前不要
开始写 ②↔③ 的接口」仍然有效。
...
**2026-09-09 更新：`C14` 的传输那一半已落地，转 ⚠️ 部分完成。**
ENet 上跑 DTLS，客户端 pin 服务器证书；门禁 `tools/dtls_check.tscn` 断言
"明文客户端连不上""拿错证书连不上"，并做过变异测试。审计文档里原来那条
"DTLS 插不进去、降 P2"的记录已被证伪 —— 它是一条**从未验证过的**判断。

**仍缺**：Android 真机、线上部署、弱网表现。所以它还不是 ✅。

#### ⚠️ 但第六节那三条 🔴 **不因此自动解除**

很容易顺手推出"链路加密了，那 JWT 就能上 ENet 了"。不对，逐条看：

| 🔴 | `C14` 之后还成不成立 |
|---|---|
| 1. JWT 不上 ENet | **仍然成立，但理由变了。** 原来的理由是"明文广播凭证"，那一条确实没了；现在的理由是**最小权限** —— ③ 不需要知道你是谁，只需要知道"这条连接拥有 3 号座"。让一个不需要身份的进程持有完整身份，等于把一次泄漏从"毁一局"升级成"丢号"。第八节已经写过这句话，**它与 `C14` 做没做无关** |
| 2. ③ 不读写持久化货币 | 完全不受影响，与链路加密无关 |
| 3. `NetProtocol` 不加账号身份字段 | 同 1 |

**第九节 `9.1` / `9.2` 一个都没被回答。** ③ 怎么验 JWT、③→② 用什么服务端凭证，
这两个问题原本"取决于 `C14` 用什么方案" —— 现在方案有了（DTLS + pin 证书），
所以它们**从"还不能定"变成了"可以定了、但还没定"**。

**这意味着第三节末尾那个「座位票」候选方案现在该拍板了。** 它此前压在 `C14`
后面，而 `C14` 已经不再是挡路的那一个。它仍然是候选、不是决议。
...
- **不动 `SessionContext.session_token`。** 那是重连凭证，与账号是两回事，
  ...
  **两者的分工（2026-09-09 补）。** 有了账号之后很容易冒出「那 `session_token`
  是不是就不需要了」的念头。不是 —— 它们答的不是同一个问题：

  | | 答什么 | 生命周期 |
  |---|---|---|
  | 账号 / JWT | **你是谁** | 永久、跨设备 |
  | `session_token` | **这条连接拥有哪个座位** | 一局一换 |

  正常情况这两个重合，所以看起来冗余。会分叉的是这几处：

  - **同一个账号从两台设备同时连进来**时，光靠 `player_id` 分不出两条连接 ——
    为裁决「座位归谁」而发明的任何东西，本质上就是 `session_token`。
  - **AI 席 / 空席 / dummy 没有账号**，但座位逻辑照常跑（服务器还要替它们代过
    result ACK，见 `C7`）。座位身份必须能独立于账号存在。
  - **离线自测与本地房主调试路径压根不联网。**
  - ③ 不需要知道你是谁，只需要知道「这条连接拥有 3 号座」。让一个不需要身份的
    进程持有完整身份，等于把一次泄漏从「毁一局」升级成「丢号」。
    **这条与 `C14` 做没做无关**，是最小权限。

  正确的关系是两层：**账号 = 座位归属的锚**（谁*有权*认领 3 号座），
  **`session_token` = 这条连接的凭证**（当前*谁在坐*）。上面第三节的「座位票」
  就是这两层的具体形状。
...
| 9.7 | 换设备 / 重装 app 后，座位怎么认领？ | `session_token` 只存在本地（`SaveManager.save_reconnect`），清数据就没了，座位再也找不回来。**这是账号能给联机栈带来的唯一实质好处**，但补它要先有 ②↔③ 通道，压在 `C14` 后面。形状见第三节「座位票」 |
