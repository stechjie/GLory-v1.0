...
| 7 | V12-08 / V12-09 AI 与经济量化 | **一致性修复与量化完成；待产品定目标** | 100-seed 经济/阵容与 700 场存活曲线已落盘；不自动改平衡 |
| 8 | V12-11 雷电/PC | **诊断入口完成；待雷电环境复现** | 已固定同 APK hash 的 30 秒取证协议；没有设备前不盲切 renderer/ABI |
...
- 已按 `glory-vfx-director` 的视觉/VFX流程核对项目，但 V12 清单引用的战斗视频、17 组接触表来源图和 `ai_battle_followup.md` 在当前仓库及共享目录均不存在。
- 没有实际参考媒体就无法诚实判断角色分离、命中锚点、材质或共享 shader 应怎样改；本阶段未修改模型、材质、VFX、相机或仿真。
- 补齐固定回放/阵容和同机截图后，再按单体、五守卫、密集战斗矩阵做表现层修复，并在改动前单独确认具体设计。

## 阶段 7 验证记录

- Bot 商店档位改为直接复用 `ShopRoll`，不再复制曲线；分布数值未变。
- Bot 升星从错误的 3/9 份改为读取 `GameConstants` 的 2/6 份真实规则；这是与真人/服务端的规则一致性修复，不是数值调参。
- 旧金币公式封装并标识为 `legacy_estimate_v1`，当前生产预算强度保持不变；真实经济的胜负前提尚未由产品定义，因此不猜一个公式上线。
- 100-seed 量化显示第 9 回合 legacy 累计预算 184，而真实同储备账本为全胜 1094、全败 1446；连败补偿使全败收入高于全胜，需产品判断是否符合追赶设计。
- 700 场真实模拟的胜率：R1 85%、R4 49%、R5 8%、R9 94%、R10 86%、R15 97%、R21 52%；第 5 回合低谷和后续陡升已量化，但未自行设目标。
- 小灵是唯一 10 金单位，没有同价对照；其 HP 接近 tier 1 平均，攻击/防御更低且无技能。出现率由前期 20%–33% 降至 R21 7%，当前证据不足以自动改价或加技能。
- `ai_economy_curve` 954/954、`ai_survival_curve` 716/716；`shop_roll_parity` 6476/6476、`merge_rule_parity` 21/21、`determinism` 111/111，均 `polluted=false`。详细数据见 `V12_AI_ECONOMY_AND_SURVIVAL_BASELINE_20260907.md`。
