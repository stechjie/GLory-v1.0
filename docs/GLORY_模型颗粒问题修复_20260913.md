...
本节记录模型修复阶段的 macOS 真实 GPU 验证，使用 Godot 4.7 和隔离的代码/资源合并项目；该阶段尚未重新生成安装包。后续已完成 Android 安装与女王备战、移动和攻击实测，见[真机显示与启动修复报告](GLORY_真机显示与启动修复_20260913.md)第 5–7 节。截图中的 FPS 是加载后冻结首帧的 HUD 数字，不能作为性能测量。
