...
### Current health (generated 2026-08-28T06:38:51Z)

| Item | Value |
| --- | --- |
| Commit | `1bcb5b6` on `main` (26 dirty tracked, 31 untracked) |
| Asset inventory | `084b5a8c59f48bfa` / 2658 files |
| Gates | 45 known, 2 failing (45 from the run at 2026-08-28T06:35:47Z, rest from earlier runs) |
| Model material integrity | 75 models, 3 white-material suspects, 1 failing |
| APK content scan | GloryBetaV10.apk: 3690 entries, clean |
| Last device run | 2026-08-21T04:26:07Z on 25028RN03A (FAILED) |

Failing gates:

- `dynamic_call_check` - nonzero_exit, 10 failures of 209 checked
- `model_material_integrity_check` - nonzero_exit, 3 failures of 300 checked

Known blockers (source: `data/qa/known_blockers.json`):

- **blocked** (external, V2 P0-03 / P0-04) - Flame Claw 缺少源贴图，12 张截图仍是白模
- **blocked** (codex, V2 A2 / 资源交付) - 资源交付包指纹已过期，restore_assets.ps1 会拒绝运行
- **in_progress** (codex, V2 P0-04 人工验收) - Virgo 自动矩阵通过，但人工可见黑色三角/透明轮廓错误
- **not_started** (unassigned, README A4) - Android 战斗回归未完成
- **not_started** (unassigned, README A5) - 第三方许可总账未完成
- **blocked** (external, V3 P0-01) - 启动 T1–T4 没有真机基线
- **not_started** (codex, dynamic_call 衍生) - 法师技能预览的动作调用恒为死分支：全仓没有 set_action
- **not_started** (unassigned, 既有) - dynamic_call 棘轮已被突破且注释自相矛盾
- **not_started** (unassigned, V2 P2-01 衍生) - tools/ 与 officetest/ 进了发布包
