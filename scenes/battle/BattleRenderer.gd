...
func _refresh_visuals() -> void:
	var update_model_facing := _model_facing_due
	...
	_sync_3d_model_nodes(living, update_model_facing)
...
func _sync_3d_model_nodes(living: Array, update_facing: bool, prune := true) -> void:
	...
	for f in living:
		...
		_position_3d_model_node(model_node, f, update_facing)
		_apply_formation_intro_visibility(id, f)
...
func _make_shared_model_node(f: Dictionary) -> Node3D:
	...
	pivot.set_meta("base_yaw", float(unit_def.get("model_base_yaw", 180.0)))
	...
	_ensure_status_vfx_controller(pivot)
...
func _ensure_status_vfx_controller(pivot: Node3D) -> Node:
...
func _position_3d_model_node(model_node: Node3D, f: Dictionary, update_facing: bool) -> void:
	var pos := _sim_to_world_pos(_visual_sim_pos_for_fighter(f))
	model_node.position = Vector3(pos.x, battle_unit_y_offset, pos.z)
	if not update_facing:
		return
	var yaw := _team_facing_yaw(model_node, f)
	model_node.rotation_degrees.y = lerp_angle(deg_to_rad(float(model_node.rotation_degrees.y)), deg_to_rad(yaw), MODEL_FACING_LERP) * 180.0 / PI

func _team_facing_yaw(model_node: Node3D, f: Dictionary) -> float:
	...
	if GameState.round_index == GameState.FINAL_ROUND:
		return base_yaw + (MODEL_FACING_PLAYER_YAW_OFFSET - 90.0 if is_player else MODEL_FACING_ENEMY_YAW_OFFSET + 90.0)
	# (4) When the arena is flipped, top/bottom swap, so the facing must swap too.
	if _arena_flip_y:
		is_player = not is_player
	var offset := MODEL_FACING_PLAYER_YAW_OFFSET if is_player else MODEL_FACING_ENEMY_YAW_OFFSET
...
func _begin_visual_frame(living: Array) -> void:
	...
	for f in living:
		if typeof(f) != TYPE_DICTIONARY:
			continue
		_frame_living_ids.append(_visual_id(f))
