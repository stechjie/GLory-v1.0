...
const BATTLE_PVP_BACKGROUND_SCALE := 1.2
...
func _build() -> void:
	...
	arena_tint.color = Color(0.045, 0.070, 0.110, 0.10) if _uses_pvp_battlefield() else Color(0.012, 0.030, 0.022, 0.22)
...
func _uses_pvp_battlefield() -> bool:
	...
	# 这里用赛程表提前判断本回合是不是 PVP（final 也走雪地），否则组队 PVP 永远是草地。
	if kind == "team" or kind == "":
		kind = RoundService.schedule_kind_for_round(GameState.round_index)
	return kind == "pvp" or kind == "final"


func _battlefield_base_path() -> String:
	return BATTLE_2_5D_PVP_BASE_PATH if _uses_pvp_battlefield() else BATTLE_2_5D_BASE_PATH
