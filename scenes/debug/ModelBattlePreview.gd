...
var unit_browser_race_select: OptionButton
var unit_browser_piece_select: OptionButton
var unit_browser_skill_button: Button
var unit_browser_attack_button: Button
var unit_browser_info_label: Label
var unit_browser_race := "god"
var unit_browser_piece_index := 0
...
func _ready() -> void:
	...
	_build_unit_browser_panel()
	_populate_unit_browser_races()
...
func _build_unit_browser_panel() -> void:
	var panel := PanelContainer.new()
	panel.name = "UnitBrowserPanel"
	...
	rows.add_child(race_row)
	var race_label := Label.new()
	race_label.text = "Race"
	race_row.add_child(race_label)
	unit_browser_race_select = OptionButton.new()
	unit_browser_race_select.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	unit_browser_race_select.item_selected.connect(_on_unit_browser_race_selected)
	race_row.add_child(unit_browser_race_select)

	var piece_row := HBoxContainer.new()
	piece_row.add_theme_constant_override("separation", 6)
	rows.add_child(piece_row)
	var piece_label := Label.new()
	piece_label.text = "Piece"
	piece_row.add_child(piece_label)
	unit_browser_piece_select = OptionButton.new()
	unit_browser_piece_select.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	unit_browser_piece_select.item_selected.connect(_on_unit_browser_piece_selected)
	piece_row.add_child(unit_browser_piece_select)

	var action_row := HBoxContainer.new()
	action_row.add_theme_constant_override("separation", 6)
	rows.add_child(action_row)
	unit_browser_skill_button = _add_button(action_row, "Skill / VFX", _on_unit_browser_skill_pressed)
	unit_browser_attack_button = _add_button(action_row, "Attack / Projectile", _on_unit_browser_attack_pressed)

	unit_browser_info_label = Label.new()
	unit_browser_info_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	unit_browser_info_label.add_theme_font_size_override("font_size", 11)
	rows.add_child(unit_browser_info_label)

func _populate_unit_browser_races() -> void:
	if unit_browser_race_select == null:
		return
	unit_browser_race_select.clear()
	var races := [
		{"id": "god", "name": "God"},
		{"id": "human", "name": "Human"},
		{"id": "dark", "name": "Dark"},
	...
	var selected := 0
	for i in races.size():
		unit_browser_race_select.add_item(str(races[i].get("name", "Race")))
		unit_browser_race_select.set_item_metadata(i, str(races[i].get("id", "")))
		if str(races[i].get("id", "")) == unit_browser_race:
			selected = i
	unit_browser_race_select.select(selected)
	_populate_unit_browser_pieces()

func _populate_unit_browser_pieces() -> void:
	if unit_browser_piece_select == null:
		return
	unit_browser_piece_select.clear()
	var entries := _unit_browser_entries_for_race(unit_browser_race)
	for i in entries.size():
		var entry: Dictionary = entries[i]
		unit_browser_piece_select.add_item(str(entry.get("name_en", entry.get("name", entry.get("id", "Piece")))))
		unit_browser_piece_select.set_item_metadata(i, str(entry.get("id", "")))
	if entries.is_empty():
		unit_browser_info_label.text = "No pieces for this race"
		return
	var preferred := 0
	for i in entries.size():
		if str(entries[i].get("id", "")) == "god_priest":
			preferred = i
			break
	unit_browser_piece_index = clampi(preferred, 0, entries.size() - 1)
	unit_browser_piece_select.select(unit_browser_piece_index)
	_load_unit_browser_piece()

func _unit_browser_entries_for_race(race: String) -> Array:
	var result: Array = []
	for value in model_entries:
		var entry: Dictionary = value
		if str(entry.get("source", "")) != "棋子":
			continue
		if str(entry.get("race", "")) == race and str(entry.get("model", "")).begins_with("res://"):
			result.append(entry)
	return result

func _on_unit_browser_race_selected(index: int) -> void:
	if unit_browser_race_select == null:
		return
	unit_browser_race = str(unit_browser_race_select.get_item_metadata(index))
	_populate_unit_browser_pieces()

func _on_unit_browser_piece_selected(index: int) -> void:
	unit_browser_piece_index = index
	_load_unit_browser_piece()

func _load_unit_browser_piece() -> void:
	var entries := _unit_browser_entries_for_race(unit_browser_race)
	if entries.is_empty():
		return
	unit_browser_piece_index = clampi(unit_browser_piece_index, 0, entries.size() - 1)
	var entry: Dictionary = entries[unit_browser_piece_index]
	selected_left_index = _find_entry_index(str(entry.get("id", "")), selected_left_index)
	selected_right_index = _find_entry_index("battle_crystal_red", selected_right_index)
	if left_model_select != null:
		left_model_select.select(selected_left_index)
	if right_model_select != null:
		right_model_select.select(selected_right_index)
	auto_fight_enabled = false
	_update_auto_button()
	_load_selected_pair()
	_update_unit_browser_info(entry)

func _update_unit_browser_info(entry: Dictionary) -> void:
	if unit_browser_info_label == null:
		return
	var skill_id := str(entry.get("skill_id", "none"))
	var piece_name := str(entry.get("name_en", entry.get("name", entry.get("id", "Piece"))))
	unit_browser_info_label.text = "%s\nSkill: %s\nTarget: fixed dummy" % [piece_name, skill_id]
...
func _load_model_entries() -> Array:
	...
	result.append({
		"id": "battle_crystal_blue",
		"name": "蓝水晶 Toon（敌方）",
		"source": "战场水晶",
		"model": BLUE_CRYSTAL_MODEL_PATH,
	...
	result.append({
		"id": "battle_crystal_red",
		"name": "红水晶 Toon（我方）",
		"source": "战场水晶",
		"model": RED_CRYSTAL_MODEL_PATH,
		"model_base_yaw": 0.0,
		"model_visual_scale": 1.0,
		"crystal_toon": true,
		"crystal_color": "red",
...
func _select_default_models() -> void:
	selected_left_index = _find_entry_index("battle_crystal_blue", 0)
	selected_right_index = _find_entry_index("battle_crystal_red", 1 if model_entries.size() > 1 else 0)
...
func _spawn_unit(slot: Node3D, entry: Dictionary, side: int) -> void:
	...
	if bool(entry.get("crystal_toon", false)):
		_apply_crystal_toon_preview(model, str(entry.get("crystal_color", "blue")))
		_add_crystal_cluster_preview(model, path, str(entry.get("crystal_color", "blue")))
	...
	var unit := {
		...
		"actions": actions,
		"current_action": "idle",
		"is_crystal": bool(entry.get("crystal_toon", false)),
		"crystal_base_yaw": model.rotation.y,
		"load_note": ""
	}
	units.append(unit)
	slot.position = unit["base_pos"]
...
func _animate_units() -> void:
	...
	for unit in units:
		...
		var is_crystal := bool(unit.get("is_crystal", false))
		var bob_phase := 0.0 if side < 0 else PI
		var bob: float = sin(phase_time * TAU / 2.6 + bob_phase) * 0.14 if is_crystal else sin(phase_time * TAU * 0.9) * 0.035
		var model := unit.get("model") as Node3D
		if is_crystal and model != null:
			var base_yaw: float = float(unit.get("crystal_base_yaw", model.rotation.y))
			model.rotation.y = base_yaw + sin(phase_time * TAU / 5.2 + bob_phase) * deg_to_rad(4.0)
			model.rotation.z = sin(phase_time * TAU / 4.1 + bob_phase) * deg_to_rad(1.6)
		var engage_pos := Vector3(base_pos.x, base_pos.y, float(side) * ENGAGE_DEPTH)
		var current_base := base_pos
		if auto_fight_enabled and not is_crystal:
			current_base = base_pos.lerp(engage_pos, walk_t)
		var lunge: float = 0.0
		if not is_crystal and (in_attack or action == "attack"):
			lunge = sin(attack_t * PI) * 0.46
		elif not is_crystal and action == "run":
...
func _apply_crystal_toon_preview(root: Node3D, color_name: String) -> void:
	var is_red := color_name == "red"
	...
	for found in mesh_nodes:
		var mesh_instance := found as MeshInstance3D
		if mesh_instance == null or mesh_instance.mesh == null:
			continue
		var original_mesh := mesh_instance.mesh
		var toon_mesh := original_mesh.duplicate() as Mesh
		if toon_mesh == null:
			continue
		...
		for surface_index in toon_mesh.get_surface_count():
			var source := original_mesh.surface_get_material(surface_index) as BaseMaterial3D
			...
			# World-space gradient span; the preview stands its crystals on y = 0.
			toon.set_shader_parameter("gradient_base_y", root.global_position.y)
			toon.set_shader_parameter("gradient_height", CRYSTAL_GRADIENT_HEIGHT)
...
func _add_crystal_cluster_preview(cluster_root: Node3D, model_path: String, color_name: String) -> void:
	var packed := ResourceLoader.load(model_path) as PackedScene
	if packed == null:
		return
	var offsets := [
		Vector3(-0.34, 0.05, 0.02), Vector3(0.36, 0.10, -0.04),
		Vector3(-0.18, 0.26, 0.18), Vector3(0.20, 0.32, 0.14),
		Vector3(-0.60, 0.72, 0.02), Vector3(0.58, 0.82, -0.08)
	]
	...
	for i in offsets.size():
		...
		_apply_crystal_toon_preview(part, color_name)
...
func _unit_browser_left_unit() -> Dictionary:
	for unit in units:
		if int(unit.get("side", 0)) == SIDE_LEFT:
			return unit
	return {}

func _on_unit_browser_attack_pressed() -> void:
	_clear_all_preview_nodes()
	auto_fight_enabled = false
	_update_auto_button()
	var unit := _unit_browser_left_unit()
	if unit.is_empty():
		if unit_browser_info_label != null:
			unit_browser_info_label.text = "Attack: no piece loaded"
		return
	var slot := unit.get("slot") as Node3D
	if slot == null or right_slot == null:
		return
	var unit_id := str(unit.get("id", ""))
	var race := _preview_visual_race(unit_id)
	var mode := "melee" if int(unit.get("range", 1)) <= 1 else "ranged"
	var origin := slot.global_position + Vector3(0.0, 0.24, 0.0)
	var target := right_slot.global_position + Vector3(0.0, 0.24, 0.0)
	var context := {
		"origin_node": slot,
		"target_node": right_slot,
		"source_unit_id": unit_id,
		"target_unit_id": str(right_slot.name),
		"targets": [target],
	}
	var composer := UNIT_SKILL_COMPOSER.new()
	composer.name = "UnitBrowserAttack_%s" % (unit_id if not unit_id.is_empty() else "unit")
	vfx_preview_root.add_child(composer)
	composer.play_skill("basic_attack_%s_%s" % [mode, race], origin, target, context)
	_play_unit_action(unit, "attack", true)
	if unit_browser_info_label != null:
		unit_browser_info_label.text = "%s\nAttack: %s %s\nTarget: fixed dummy" % [str(unit.get("name", unit_id)), race, mode]

func _on_unit_browser_skill_pressed() -> void:
	_clear_all_preview_nodes()
	auto_fight_enabled = false
	_update_auto_button()
	var unit := _unit_browser_left_unit()
	if unit.is_empty():
		if unit_browser_info_label != null:
			unit_browser_info_label.text = "Skill: no piece loaded"
		return
	var skill_id := str(unit.get("skill_id", "none"))
	if skill_id.is_empty() or skill_id == "none":
		if unit_browser_info_label != null:
			unit_browser_info_label.text = "%s\nSkill: none" % str(unit.get("name", unit.get("id", "Piece")))
		return
	var slot := unit.get("slot") as Node3D
	if slot == null or right_slot == null:
		return
	...
	if skill_id == "lowest_ally_heal":
		# Preview mode shows the full Star Prayer package, including its
	...
	composer.play_skill(skill_id, origin, target, context)
	if unit_browser_info_label != null:
		unit_browser_info_label.text = "%s\nSkill: %s\nTarget: fixed dummy (status follows target)" % [str(unit.get("name", unit.get("id", "Piece"))), skill_id]
