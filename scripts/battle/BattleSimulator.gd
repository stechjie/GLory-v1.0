...
# Boss（boss_*）与法阵守护者（ally_*）也在这里排除。9.25 之前只排 ally 没排 boss，
# 理由是「boss 由 _nearest_non_boss 拦」—— 行为上确实拦住了（本函数只有一个调用点，
# 结果直接喂给 _nearest_non_boss），但函数名承诺的是「可连目标池」，
...
# 法阵守护者（第 21 回合按法阵血量召唤的友军，ally_*）。9.25 反馈第 1 条：
# 末日守卫的血链能把这些法阵 boss 策反过来。它们既不是 `boss_*`（_is_boss_fighter
# 抓不到），也不是玩家棋子，被血链一转换就白拿一只高血守护者、连接本身也失去意义，
# 与「Boss 免疫」的既有口径同类，因此血链候选池必须显式排除。
# 与 _ignores_treasure 同一个 `is_formation_ally` 字段（BattleSimShared._fighter_from_def
