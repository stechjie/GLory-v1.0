...
const LIMITS := {
	...
	#
	# 2026-09-09 影子期上线时从 60 提到 120：这个额度是按「客户端一条经济 intent
	# 都不发」的年代定的。双写之后每一次买入都是 1~2 条（买入 + 可能的合成），
	# 连点买三张卡就是 6 条。撞上限的后果不是丢一条记账那么轻 ——
	# STRIKES_BEFORE_KICK 是 3，连续超限会**把玩家踢下线**。
	"economy": 120,
	...
	"chat_text": 3,
	# 语音钥匙（docs/语音LiveKit方案.md）：正常一局只要几次（开语音、换队、断线重连）；
	# 客户端连不上时按 2 / 4 / 8 / 15 / 30 秒退避重试，10 秒 5 次足够。
	# **不计 strike**（调用处传 count_strike=false）：重试是网络不好，不是攻击。
	"voice_token": 5,
