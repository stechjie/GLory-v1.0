...
> 当前结论（2026-08-19）：**A1、A2、A3、E1、E2-2A/E2-2B 与 BattlePresentationDirector D0-D4 的 Windows/桌面闭环已完成。** A4 及各项 Android/实机部分按当前决定暂停；Director 下一项是 D5 手感与运行时预算，D5-D6 尚未完成。后文关于“35 项不可加载”和“胶囊占位体”的文字是改造前历史基线，不再代表当前桌面结果。
...
| Director D5-D6 | ⬜ 下一阶段 | 下一项是 D5 手感与运行时预算（五个 profile、质量预算、评审场景、切片扩面）；之后 D6 清理旧路由。 |
...
### 当前下一步：Director D5-D6

1. D5：加入五个 profile、`VFXQualityBudget`、质量降级和评审场景；Android 评审仍保持暂停。
2. D6：逐类型清理 `BattleVfx` 旧路由，验证无重复播放、跨局残留及节点/内存持续增长。
...
4. Director D0-D4 桌面闭环：**已完成。** 当前进入 D5 手感与运行时预算；之后完成 D6 与 B2/B3 演出纵向切片的扩面。
