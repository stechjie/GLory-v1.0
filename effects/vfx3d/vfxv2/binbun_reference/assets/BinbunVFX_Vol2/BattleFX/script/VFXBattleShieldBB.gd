...
## Exponent used on the noise texture
@export var noise_twist : float = 1.0:
	set(v):
		noise_twist = v
		_set_shader_param("noise_twist", noise_twist)

## Exponent used on the noise texture
@export var orbital_rotation : float = 0.0:
	set(v):
		orbital_rotation = v
		_set_shader_param("rotation_amount", orbital_rotation)
...
## Amount of particles emitted by this effect
@export var particle_amount : int = 64:
	set(v):
		particle_amount = v
		_set_particle_param("amount", particle_amount)

## If [code]true[/code], particles use the parent node's coordinate space. Otherwise particles use global coordinates
@export var particle_local_coords : bool = false:
	set(v):
		particle_local_coords = v
		_set_particle_param("local_coords", particle_local_coords)

## The framerate at which particles are rendered. Higher values can decrease performance.
@export_custom(PROPERTY_HINT_NONE, "suffix:FPS") var particle_fps : int = 30:
	set(v):
		particle_fps = v
		_set_particle_param("fixed_fps", particle_fps)
