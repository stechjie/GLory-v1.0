...
func _poison_attack(origin:Vector3,target:Vector3,context:Dictionary)->void:
	...
	dart.play_layer("res://assets/vfx/skills/undead_spike/undead_spike_bone_dart.png",{"from":origin+Vector3(0,.30,0),"to":target+Vector3(0,.34,0),"size":Vector2(.72,.24),"duration":.78,"travel_ratio":.68,"dark_tint":dart_profile.dark_color,"body_tint":dart_profile.main_color,"core_tint":dart_profile.core_color,"seed":4.6,"flow_strength":.022})
...
func _black_hole(origin:Vector3)->void:
	var p:=_shadow_profile(1.34,2.35);p.main_color=Color(.20,.028,.42);p.core_color=Color(.74,.22,1.0);p.particle_count=12;p.emission_energy=3.8
	_spawn(VFX_VORTEX,p,{"target":origin})
	var collapse:=VFX_IMPACT_FLASH.new();add_child(collapse)
	var cp:=_profile(Color(.015,.003,.035),Color(.20,.025,.46),Color(.78,.30,1.0),.66,.48,3.6,6)
	var tw:=create_tween();tw.tween_interval(1.32);tw.tween_callback(func():
		if is_instance_valid(collapse):collapse.play_profile(cp,{"target":origin+Vector3(0,.04,0)})
	)
