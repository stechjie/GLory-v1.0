...
func play_slash(at: Vector3, direction: Vector3, profile: VFXProfile3D = null) -> void:
	...
	main_profile.parameters["width"] = float(params.get("width", 0.30))
	...
	echo_profile.emission_energy *= 0.62
	echo_profile.parameters = echo_profile.parameters.duplicate()
	echo_profile.parameters["width"] = float(params.get("width", 0.30)) * 0.58
