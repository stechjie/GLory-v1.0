...
func play_status(at:Vector3,status_type:String,profile:VFXProfile3D=null,target_node:Node3D=null)->void:
	...
	for i in range(3):
		var orbit:=_make_card("StatusOrbit_%d"%i,TEX_WISP,Vector2(s*.24,s*.64),active,float(i)*2.7)
		orbit.position.y=s*(.50+.06*float(i%2));orbit.scale=Vector3.ONE*.09
		_animate_orbit(orbit,active,i)
...
func _update_anchor()->void:
	...
	else:position=_fallback_target
