...
void fragment(){vec2 p=UV-vec2(.5);float r=length(p)*2.0;float n=texture(noise_tex,UV*2.0+vec2(TIME*.07,phase)).r;float angle=atan(p.y,p.x)*.15915+.5;float tear=texture(noise_tex,vec2(angle*2.2+phase,r*.72+TIME*.035)).r;float mask=texture(mask_tex,rot(p,TIME*speed+phase)+vec2(.5)).a;float hollow=smoothstep(.10+.08*layer,.33+.11*layer,r+tear*.10);float edge=smoothstep(1.08,0.74+(n-.5)*.18,r);float broken=smoothstep(.18,.72,tear+n*.35);float hot=pow(mask,2.2)*(1.0-layer*.58);float fade=smoothstep(0.0,.10,life)*(1.0-smoothstep(.76,1.0,life));vec3 c=mix(dark_color.rgb,main_color.rgb,mask);c=mix(c,core_color.rgb,hot);ALBEDO=c;EMISSION=c*energy*(.22+hot*1.35);ALPHA=mask*hollow*edge*broken*fade*opacity*mix(.88,.38,layer);}
"""
...
func play_vortex(at:Vector3,profile:VFXProfile3D=null)->void:
	...
	var shadow:=_ground_layer("VortexDarkMass",TEX_SWIRL,Vector2(s*2.45,s*1.68),active,0.0,.34,0);shadow.scale=Vector3.ONE*.16
	var body:=_ground_layer("VortexFlowBody",TEX_SWIRL,Vector2(s*2.08,s*1.42),active,2.4,-.82,1);body.scale=Vector3.ONE*.10
	var rim:=_ground_layer("VortexRaggedRim",TEX_RING,Vector2(s*2.38,s*1.60),active,4.7,.46,2);rim.scale=Vector3.ONE*.08
	var core:=_ground_layer("VortexIrregularCore",TEX_SWIRL,Vector2(s*.86,s*.62),active,-2.6,1.15,3);core.scale=Vector3.ONE*.12
	...
	var core_tween:=track_tween(create_tween());core_tween.tween_interval(d*.12);core_tween.tween_property(core,"scale",Vector3.ONE*1.18,d*.18).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT);core_tween.tween_property(core,"scale",Vector3.ONE*.72,d*.30).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_IN)
	_tween_life(shadow,d*1.02,0.0);_tween_life(body,d*.94,d*.035);_tween_life(rim,d*.86,d*.08)
	_tween_life(core,d*.62,d*.12)
...
func _spawn_ground_tendrils(profile:VFXProfile3D)->void:
	for arm in range(7):
		var verts:=PackedVector3Array();var uvs:=PackedVector2Array();var indices:=PackedInt32Array();var segments:=24;var phase:=float(arm)/7.0*TAU;var width:=profile.size*(.115+.022*float(arm%2))
