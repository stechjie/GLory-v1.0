...
`/v1/auth/anonymous` 是唯一会**凭空造出数据**的公开接口 —— 每调一次就在
Supabase 多一个用户、在 `players` 多一行。不限流的话任何人写个循环就能撑爆
数据库并把 MAU 账单顶上去（Supabase Dashboard 自己也提醒过）。

Supabase 建议给匿名登录挂 captcha。我们不走那条：captcha 塞进 Godot 很难受，
而且**本来就不需要** —— publishable key 不下发给客户端，外面的人打不到
Supabase 的注册接口，只能走我们这个端点。入口只有一个，在这里限流就够。

额度可配（见 `.env.example`），默认注册 10/小时、刷新 120/小时。
两者不能写反：刷新是每次启动的正常动作，注册一辈子只该有一次 ——
写反了正常玩家会在启动时被限流，而那看起来像服务器坏了。有测试钉着这条。

### 两个必须知道的局限

1. **计数在进程内存里。** 开多个 worker 或多个实例时每个进程各算各的，
   实际额度会翻倍。真上量后要换 Redis 或在反向代理层做。
2. **来源识别只信任本机回环转发的 `X-Forwarded-For`。** 见 `routes/auth.py`
   的 `_client_ip()`：直连本服务的请求一律按真实对端计，只有确认来自同机的
   Caddy 才采信那个头 —— 它是客户端可以随便写的，盲信比不限流还糟。
   仍挡不住同一出口 IP 共用额度（NAT）与手握大量 IP 的攻击者。

限流在 `_require_db()` **之前**生效：要在做任何工作之前就挡住，
...
| 6. 限流 + 接进启动流程 | ✅ 限流已做；自动登录**默认关闭**，等后端部署 |
