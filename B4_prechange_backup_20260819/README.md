...
- **状态（2026-08-19）：优先级预算已落地，满配压力验收未做。** D5 已在 `VFXQualityBudget.gd` 内追加 `critical / important / ambient` 的每 tick 起播上限、存活上限、合并窗口与降级系数，规则是 critical 不设限不合并不缩短、important 先让出收招、ambient 仅在溢出且同目标重复时合并；固定样本实测 0 合并、20 次降级。逐帧指标（fps、draw call、节点、orphan、tween、显存）已写入 `frame_performance.csv`。**未做的是压力面**：粒子与透明叠层上限尚未纳入优先级预算，「低档 + 满配第 21 回合」这一场也还没跑过。
...
- **状态（2026-08-19）：场景已建，对照功能未做。** `scenes/debug/BattleVfxReview.tscn` 可固定 seed、选回合、切 `LOW/MEDIUM/HIGH` 与 `0.25×/0.5×/1×/2×`，一键保存截图与指标 json。**「旧 profile / 新 profile 并排比较」尚未实现**——当前只能分两次运行再人工比对保存下来的截图与 json。
