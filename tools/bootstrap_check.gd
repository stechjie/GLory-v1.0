...
const MISSING_PATH := "res://tools/fixtures/BootstrapTargetMissing.tscn"
...
func _new_bootstrap(path: String) -> Control:
	var bootstrap := BootstrapScene.instantiate() as Control
	bootstrap.set("auto_start", false)
	bootstrap.set("auto_transition", false)
	bootstrap.set("next_scene_path", path)
	add_child(bootstrap)
	return bootstrap
...
func _check_failure_and_retry() -> void:
	...
	var failed_snap := bootstrap.call("snapshot") as Dictionary
	...
	bootstrap.set("next_scene_path", FIXTURE_PATH)
	bootstrap.call("retry")
	var recovered := await _wait_for_phase(bootstrap, "READY")
	_h.expect(recovered, "retry_never_ready", "修正路径后 Retry 没有恢复")
	var recovered_snap := bootstrap.call("snapshot") as Dictionary
...
func _wait_for_phase(bootstrap: Control, wanted: String) -> bool:
	for _frame in 180:
