...
func _build_rest(root: VBoxContainer) -> void:
	...
	_shop_scroll_idle.setup(
		shop_open_btn,
		_cached_texture(SHOP_IDLE_ATLAS_PATH),
		_cached_texture(SHOP_IDLE_HALO_PATH)
	)
	...
	_shop_refresh_burn.setup(
		_shop_background,
		_refresh_shop_button,
		_cached_texture(SHOP_REFRESH_FIRE_ATLAS_PATH),
...
func _build_top_actions() -> void:
	...
	var team_mercs_btn := _make_framed_text_button("", TEAM_MERCS_BTN_PATH, MERC_BTN_SIZE, 16, _toggle_team_mercs_picker)
...
func _build_team_mercs_overlay(center_host: Control) -> void:
	...
	stage_background.texture = _cached_texture(TEAM_MERCS_STAGE_BACKGROUND_PATH)
	...
	var edge_outline := Panel.new()
	edge_outline.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	edge_outline.mouse_filter = Control.MOUSE_FILTER_IGNORE
	var edge_style := StyleBoxFlat.new()
	edge_style.bg_color = Color(0.0, 0.0, 0.0, 0.0)
	edge_style.border_color = Color(0.0, 0.0, 0.0, 1.0)
	edge_style.set_border_width_all(3)
	edge_outline.add_theme_stylebox_override("panel", edge_style)
	stage_holder.add_child(edge_outline)
