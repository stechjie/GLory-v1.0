...
const ACTION_SCENES := {
	"idle": "res://assets/models/monsters/ren/pve_ren_wandering_swordsman_animated/pve_ren_wandering_swordsman_idle.fbx",
	"attack": "res://assets/models/monsters/ren/pve_ren_wandering_swordsman_animated/pve_ren_wandering_swordsman_attack.fbx",
	"run": "res://assets/models/monsters/ren/pve_ren_wandering_swordsman_animated/pve_ren_wandering_swordsman_walking.fbx",
}
const BODY_MATERIAL_PATH := "res://assets/models/monsters/ren/pve_ren_wandering_swordsman_animated/pve_ren_wandering_swordsman_body_material.tres"
...
func _load_action_models() -> void:
	...
	for action in load_actions:
		...
		if scene == null:
			push_warning("古树长者动作 FBX 加载失败：%s" % path)
			continue
		var node := scene.instantiate() as Node3D
		if node == null:
			push_warning("古树长者动作 FBX 实例化失败：%s" % path)
			continue
		...
		if player == null:
			push_warning("古树长者动作缺少 AnimationPlayer：%s" % path)
...
func _activate_action(action: String, restart: bool) -> void:
	if action_nodes.is_empty():
		push_warning("古树长者没有加载到动作子模型，请检查 ACTION_SCENES 路径和 FBX .import。")
		return
	if not action_nodes.has(action):
		push_warning("古树长者缺少动作子模型：%s，回退 idle。" % action)
		action = "idle"
		if not action_nodes.has(action):
			return
