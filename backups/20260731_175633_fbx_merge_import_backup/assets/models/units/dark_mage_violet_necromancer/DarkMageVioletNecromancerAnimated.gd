...
const ACTION_SCENES := {
	"idle": "res://assets/models/units/dark_mage_violet_necromancer/dark_mage_idle.fbx",
	"attack": "res://assets/models/units/dark_mage_violet_necromancer/dark_mage_attack.fbx",
	"run": "res://assets/models/units/dark_mage_violet_necromancer/dark_mage_run.fbx"
}
const BODY_MATERIAL_PATH := "res://assets/models/units/dark_mage_violet_necromancer/dark_mage_body_material.tres"
...
func _load_action_models() -> void:
	...
	for action in load_actions:
		...
		if scene == null:
			push_warning("暗影法师动作 FBX 加载失败：%s" % path)
			continue
		var node := scene.instantiate() as Node3D
		if node == null:
			push_warning("暗影法师动作 FBX 实例化失败：%s" % path)
			continue
		...
		if player == null:
			push_warning("暗影法师动作缺少 AnimationPlayer：%s" % path)
...
func _activate_action(action: String, restart: bool) -> void:
	if action_nodes.is_empty():
		push_warning("暗影法师没有加载到任何动作子模型，请检查 ACTION_SCENES 路径和 FBX .import。")
		return
	if not action_nodes.has(action):
		push_warning("暗影法师缺少动作子模型：%s，回退 idle。" % action)
		action = "idle"
		if not action_nodes.has(action):
			return
...
func _play_internal_animation(action: String, restart: bool) -> void:
	...
	var start_time := _action_start_time(action, player, animation_name)
	if start_time > 0.0:
		player.seek(start_time, true)
	if action == "attack":
		player.advance(0.001)

func _action_start_time(action: String, player: AnimationPlayer, animation_name: String) -> float:
	var start_frame := float(ACTION_START_FRAMES.get(action, 0.0))
	if start_frame <= 0.0 or ANIMATION_SOURCE_FPS <= 0.0:
		return 0.0
	var animation := player.get_animation(animation_name)
	var start_time := start_frame / ANIMATION_SOURCE_FPS
	if animation != null:
		start_time = clampf(start_time, 0.0, maxf(animation.length - 0.001, 0.0))
	return start_time
