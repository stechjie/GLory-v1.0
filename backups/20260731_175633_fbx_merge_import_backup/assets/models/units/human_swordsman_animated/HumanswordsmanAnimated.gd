...
const ACTION_SCENES := {
	"idle": "res://assets/models/units/human_swordsman_animated/human_swordsman_idle.fbx",
	"attack": "res://assets/models/units/human_swordsman_animated/human_swordsman_attack.fbx",
	"run": "res://assets/models/units/human_swordsman_animated/human_swordsman_run.fbx"
}
const BODY_MATERIAL_PATH := "res://assets/models/units/human_swordsman_animated/human_swordsman_toon_material.tres"
...
func _load_action_models() -> void:
	var material := ResourceLoader.load(BODY_MATERIAL_PATH, "", ResourceLoader.CACHE_MODE_REPLACE) as Material
	...
	for action in load_actions:
		...
		if scene == null:
			push_warning("魅魔动作 FBX 加载失败：%s" % path)
			continue
		var node := scene.instantiate() as Node3D
		if node == null:
			push_warning("魅魔动作 FBX 实例化失败：%s" % path)
			continue
		...
		if player == null:
			push_warning("魅魔动作缺少 AnimationPlayer：%s" % path)
...
func _activate_action(action: String, restart: bool) -> void:
	if action_nodes.is_empty():
		push_warning("魅魔没有加载到任何动作子模型，请检查 ACTION_SCENES 路径和 FBX .import。")
		return
	if not action_nodes.has(action):
		push_warning("魅魔缺少动作子模型：%s，回退 idle。" % action)
		action = "idle"
		if not action_nodes.has(action):
			return
