...
const ACTION_SCENES := {
	"idle": "res://assets/models/units/dark_dragon_animated/idle.fbx",
	"attack": "res://assets/models/units/dark_dragon_animated/attack.fbx",
	"run": "res://assets/models/units/dark_dragon_animated/run.fbx"
}
const BODY_MATERIAL_PATH := "res://assets/models/units/dark_dragon_animated/dark_dragon_body_material.tres"
...
func _load_action_models() -> void:
	...
	for action in load_actions:
		...
		if scene == null:
			push_warning("黑龙动作 FBX 加载失败：%s" % path)
			continue
		var node := scene.instantiate() as Node3D
		if node == null:
			push_warning("黑龙动作 FBX 实例化失败：%s" % path)
			continue
		...
		if player == null:
			push_warning("黑龙动作缺少 AnimationPlayer：%s" % path)
...
func _activate_action(action: String, restart: bool) -> void:
	if action_nodes.is_empty():
		push_warning("黑龙没有加载到任何动作子模型，请检查 ACTION_SCENES 路径和 FBX .import。")
		return
	if not action_nodes.has(action):
		push_warning("黑龙缺少动作子模型：%s，回退 idle。" % action)
		action = "idle"
		if not action_nodes.has(action):
			return
