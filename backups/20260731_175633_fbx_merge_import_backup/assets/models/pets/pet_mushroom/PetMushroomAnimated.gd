extends Node3D
# 宠物动画模型，结构与 assets/models/units/*_animated 的 30 个单位保持一致。
# 与单位的唯一差别：宠物目前没有 idle.fbx，所以 idle 复用 attack 的模型并
# 冻结在 IDLE_FREEZE_FRAME 这一帧当静止姿势（见 FROZEN_ACTIONS）。
# 以后补了真 idle.fbx：把 ACTION_SCENES["idle"] 指过去、删掉 FROZEN_ACTIONS 即可，其余不用动。

const ACTION_SCENES := {
	"idle": "res://assets/models/pets/pet_mushroom/pet_mushroom_attack.fbx",
	"attack": "res://assets/models/pets/pet_mushroom/pet_mushroom_attack.fbx",
	"run": "res://assets/models/pets/pet_mushroom/pet_mushroom_run.fbx"
}
const BODY_MATERIAL_PATH := "res://assets/models/pets/pet_mushroom/pet_mushroom_body_material.tres"
const ANIMATION_SOURCE_FPS := 30.0
const IDLE_FREEZE_FRAME := 0.0
const ACTION_START_FRAMES := {
	"idle": IDLE_FREEZE_FRAME
}
# 这些动作播到起始帧就暂停，不循环播放
...
func _process(_delta: float) -> void:
	...
	if current_action == "attack" and not animation_player.is_playing():
		play_idle()
		return
	# 冻结动作是故意停住的，不要每帧把它重新播起来
	if FROZEN_ACTIONS.has(current_action):
		return
...
func _load_action_models() -> void:
	...
	for action in ["idle", "attack", "run"]:
		...
		if scene == null:
			push_warning("蘑菇动作 FBX 加载失败：%s" % path)
			continue
		var node := scene.instantiate() as Node3D
		if node == null:
			push_warning("蘑菇动作 FBX 实例化失败：%s" % path)
			continue
		...
		# 空模型（FBX 导出漏了网格）不登记，让 _activate_action 回退到 idle
		if not _has_mesh(node):
			push_warning("动作 FBX 里没有网格，已跳过：%s" % path)
			node.queue_free()
			continue
		action_nodes[action] = node
		var player := _find_animation_player(node)
		if player == null:
			push_warning("蘑菇动作缺少 AnimationPlayer：%s" % path)
...
func _activate_action(action: String, restart: bool) -> void:
	if action_nodes.is_empty():
		push_warning("蘑菇没有加载到任何动作子模型，请检查 ACTION_SCENES 路径和 FBX .import。")
		return
	if not action_nodes.has(action):
		push_warning("蘑菇缺少动作子模型：%s，回退 idle。" % action)
		action = "idle"
		if not action_nodes.has(action):
			return
...
func _play_internal_animation(action: String, restart: bool) -> void:
	...
	if ACTION_START_FRAMES.has(action):
		var start_time := float(ACTION_START_FRAMES[action]) / ANIMATION_SOURCE_FPS
		var animation := player.get_animation(animation_name)
		if animation != null:
			start_time = clampf(start_time, 0.0, max(0.0, animation.length - 0.001))
...
func _best_animation_name(player: AnimationPlayer, action: String) -> String:
	...
	if best_animation != null:
		best_animation.loop_mode = Animation.LOOP_LINEAR if action == "run" else Animation.LOOP_NONE
	return best_name
...
func _has_mesh(root: Node) -> bool:
	...
	while not stack.is_empty():
		...
		if node is MeshInstance3D and (node as MeshInstance3D).mesh != null:
			if (node as MeshInstance3D).get_aabb().size.length() > 0.0:
				return true
	return false
