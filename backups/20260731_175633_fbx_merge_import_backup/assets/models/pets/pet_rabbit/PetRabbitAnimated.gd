...
const ACTION_SCENES := {
	"idle": "res://assets/models/pets/pet_rabbit/pet_rabbit_attack.fbx",
	"attack": "res://assets/models/pets/pet_rabbit/pet_rabbit_attack.fbx",
	"run": "res://assets/models/pets/pet_rabbit/pet_rabbit_run.fbx"
}
const BODY_MATERIAL_PATH := "res://assets/models/pets/pet_rabbit/pet_rabbit_body_material.tres"
...
func _load_action_models() -> void:
	...
	for action in ["idle", "attack", "run"]:
		...
		if scene == null:
			push_warning("兔子动作 FBX 加载失败：%s" % path)
			continue
		var node := scene.instantiate() as Node3D
		if node == null:
			push_warning("兔子动作 FBX 实例化失败：%s" % path)
			continue
		...
		if player == null:
			push_warning("兔子动作缺少 AnimationPlayer：%s" % path)
...
func _activate_action(action: String, restart: bool) -> void:
	if action_nodes.is_empty():
		push_warning("兔子没有加载到任何动作子模型，请检查 ACTION_SCENES 路径和 FBX .import。")
		return
	if not action_nodes.has(action):
		push_warning("兔子缺少动作子模型：%s，回退 idle。" % action)
		action = "idle"
		if not action_nodes.has(action):
			return
