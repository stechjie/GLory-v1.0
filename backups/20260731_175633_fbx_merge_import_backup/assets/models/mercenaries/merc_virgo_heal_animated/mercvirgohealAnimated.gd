...
const ACTION_SCENES := {
	"idle": "res://assets/models/mercenaries/merc_virgo_heal_animated/merc_virgo_heal_Idle.fbx",
	"attack": "res://assets/models/mercenaries/merc_virgo_heal_animated/merc_virgo_heal_attack.fbx",
	"run": "res://assets/models/mercenaries/merc_virgo_heal_animated/merc_virgo_heal_Walking.fbx",
}
const BODY_MATERIAL_PATH := "res://assets/models/mercenaries/merc_virgo_heal_animated/merc_virgo_heal_body_material.tres"
const ACTION_MODEL_SCALE := 100.0
...
func _load_action_models() -> void:
	...
	for action in load_actions:
		...
		if scene == null:
			push_warning("甲壳守卫动作 FBX 加载失败：%s" % path)
			continue
		var node := scene.instantiate() as Node3D
		if node == null:
			push_warning("甲壳守卫动作 FBX 实例化失败：%s" % path)
			continue
		node.name = "%s_model" % action.capitalize()
		node.scale *= ACTION_MODEL_SCALE
		node.visible = false
		...
		if player == null:
			push_warning("甲壳守卫动作缺少 AnimationPlayer：%s" % path)
...
func _activate_action(action: String, restart: bool) -> void:
	if action_nodes.is_empty():
		push_warning("甲壳守卫没有加载到动作子模型，请检查 ACTION_SCENES 路径和 FBX .import。")
		return
	if not action_nodes.has(action):
		push_warning("甲壳守卫缺少动作子模型：%s，回退 idle。" % action)
		action = "idle"
		if not action_nodes.has(action):
			return
...
func _action_aliases(action: String) -> Array[String]:
	match action:
		"idle":
			return ["idle", "idel", "stand", "breath", "relax"]
		"attack":
			return ["attack", "punch", "slash", "hit", "cast", "spell"]
		"run":
